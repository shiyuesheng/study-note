...
编译后的class多了一个私有静态方法，尝试将字节码翻译成java代码，如下：
...
简单分析下：该内部类类名 LambdaTest$$Lambda$1，实现了函数式接口Function 的apply方法，方法内部调用的上文提到的，该内部类所在的声明类，编译后所生成的私有静态方法。

到这，就可以把lambda云信逻辑串联起来了。
...
简单分析下：该内部类类名 LambdaTest$$Lambda$1，实现了函数式接口Function 的apply方法，

内置了一个**Field private final LambdaTest arg$1** 这个实例属性在动态调用内部类实例化时，捕获自声明类上下文中的变量

函数式接口方法内部调用的上文提到的私有静态方法，同时传入捕获的变量。前后呼应。梳理下存在捕获变量的lambda流程

- 编译时lambda表达式会在当前声明类，生成一个私有静态方法，该方法逻辑与lambda逻辑相同。如果lambda表达式有捕获变量，则方法会多出与对应类型的参数。
- 运行时动态的使用asm字节码工具生成一个实现了目标函数式接口的内部类。如果lambda表达式有捕获变量，则内部类中会多出与之对应的属性。（***此处大胆假设，因为不管执行多少次，生成的内部类class都是一样的，所以运行时应该要优化成只执行一次***）
- 内部类进行实例化，方法间传递的是内部类的实例。（***此处大胆假设，因为捕获了上下文中的变量，而这个变量在运行时是变化的，不确定的，所以每次调用都生成的新的实例***）
- 调用内部类实例的函数式接口方法（SAM Single Abstract Method）
- 内部继续调用了声明类的私有静态方法，执行我们编写的真正的lambda表达式逻辑
